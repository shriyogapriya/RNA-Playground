...
DPAlgorithm_pkcanonical.Tables[1].computeCell = function(i, j) {
    ...
    if ((j - i > this.minLoopLength) && RnaUtil.areComplementary(this.sequence[i - 1], this.sequence[j - 1])) {
        // get value for base pair
        this.updateCell(curCell, this.getValue(i+1,j-1)+1, Object.create(NussinovCellTrace).init([[i + 1, j - 1]], [[i, j]]));
    }
...
DPAlgorithm_pkcanonical.Tables[0].computeCell = function(i, j) {
    ...
    for (var k = i; k + this.minLoopLength < j; k++) {
        // check if sequence positions are compatible
        if (RnaUtil.areComplementary(this.sequence[k - 1], this.sequence[j - 1])) {
            this.updateCell(curCell, this.getValue(i,k-1)+this.getValue(k+1,j-1)+1,Object.create(NussinovCellTrace).init([[i, k - 1], [k + 1, j - 1]], [[k, j]]));
        }
    }
...
DPAlgorithm_pkAkutsu.Tables[1].computeCell = function(i0, k0) {
    ...
    {
        curCell.traces =[];
        curCell.value = maxVal;
        pkBasePairs.push(maxMatrix == 'L' | 'R');
        if(maxBP > maxVal)
        {
            maxVal -= 1;
        }
        curCell.traces.push(maxVal);
    }
